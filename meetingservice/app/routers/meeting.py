...
from app.schemas.meeting import CreateMeetingSchema, UpdateMeetingSchema
...
@meeting_router.post("/meetings/{meeting_id}/users/{user_id}")
async def add_user(
    meeting_id: int,
    email: str,
    meeting_service: MeetingService = Depends(get_meeting_service),
    ):
    return await meeting_service.add_user(meeting_id, email)